...
<body>
	<div class="container-fluid">
		<%
			...
			System.out.println("셀러리 검색 카테고리..." + col);
			System.out.println("셀러리 검색어..." + searchWord);
			...
			ArrayList<Salaries> list = new ArrayList<Salaries>();//현재 패이지 행을 저장할 변수
			PreparedStatement stmt1 = null;//쿼리문 저장할 변수
			ResultSet rs1 = null;//쿼리문에 의한 결과 상태 저장
			if (searchWord == "") {
				...
				System.out.println("검색어없음 확인");
			} else {
				
				if (col.equals("empNo")) {
					stmt1 = conn.prepareStatement(
							"select emp_no, salary, from_date, to_date from salaries where emp_no like ? order by emp_no asc limit ?,?");
					//쿼리문 : employees테이블에 데이터 (번호, 생일, 성씨, 이름, 성별, 고용날자)를 불러와서 번호순서대로 ?부터 ?까지 나열하시오
					...
					stmt1 = conn.prepareStatement(
							"select emp_no, salary, from_date, to_date from salaries where salary like ? order by emp_no asc limit ?,?");
					//쿼리문 : employees테이블에 데이터 (번호, 생일, 성씨, 이름, 성별, 고용날자)를 불러와서 번호순서대로 ?부터 ?까지 나열하시오
					stmt1.setString(1, "%" + searchNo + "%");//첫번째 물음표에 들어갈 변수
					stmt1.setInt(2, beginRow);//첫번째 물음표에 들어갈 변수
					stmt1.setInt(3, rowPerPage);//두번째 물음표에 들어갈 변수
					System.out.println("봉급검색 선택 확인");
				}
			}
			rs1 = stmt1.executeQuery();//stmt1쿼리 실행 후 저장
			while (rs1.next()) {//쿼리 실행 결과 데이터가 있으면 계속 반복
				Salaries s = new Salaries();//쿼리 실행결과 데이터로 리스트 작성
			...
			ResultSet rs2 = null;
			if (searchWord == "") {
				stmt2 = conn.prepareStatement("select count(*) from salaries");//데이터의 갯수를 구하라
			} else {
				
				if (col.equals("empNo")) {
					stmt2 = conn.prepareStatement("select count(*) from salaries where emp_no like ?");
					stmt2.setString(1, "%" + searchNo + "%");//첫번째 물음표에 들어갈 변수
				} else {
					stmt2 = conn.prepareStatement("select count(*) from salaries where salary like ?");
		...
		<div class="row">
			...
			<div class="col-sm-8">
				...
				<div>
					<form
						action="<%=request.getContextPath()%>/salaries/salariesList.jsp"
						method="post">
						<select name="col">
							<option value="salary">봉급검색</option>
				...
				<div class="d-flex justify-content-center">
					<!-- 1페이지로 이동 -->
					<a
						href="<%=request.getContextPath()%>/salaries/salariesList.jsp?currentPage=1"
					...
					%>
					<a
						href="<%=request.getContextPath()%>/salaries/salariesList.jsp?currentPage=<%=currentPage - 1%>&searchWord=<%=searchWord%>&col=<%=col%>"
						class="
			btn btn-outline-primary"><</a>
					...
					%>
					<a
						href="<%=request.getContextPath()%>/salaries/salariesList.jsp?currentPage=<%=i%>&searchWord=<%=searchWord%>&col=<%=col%> "
						class="
			...
			btn btn-outline-primary">></a>
					...
					<!-- 마지막 페이지로 이동 -->
					<a
						href="<%=request.getContextPath()%>/salaries/salariesList.jsp?currentPage=<%=lastPage%>&searchWord=<%=searchWord%>&col=<%=col%>"
						class="
