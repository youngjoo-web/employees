...
<body>
	<div class="container-fluid">
		<%
			...
			System.out.println("부서별메니저 검색키워드.." + col);
			System.out.println("부서별메니저 검색단어..." + searchWord);
			...
			ArrayList<DeptManager> list = new ArrayList<DeptManager>();//현재 패이지 행을 저장할 변수
			PreparedStatement stmt1 = null;//쿼리문 저장할 변수
			ResultSet rs1 = null;//쿼리문에 의한 데이터값 저장할 변수
			if (searchWord == "") {
				stmt1 = conn.prepareStatement(
						"select emp_no, dept_no, from_date, to_date from dept_manager order by emp_no asc limit ?,?");
				...
				if (col.equals("deptNo")) {
					stmt1 = conn.prepareStatement(
							"select emp_no, dept_no, from_date, to_date from dept_manager where dept_no like ? order by emp_no asc limit ?,?");
					stmt1.setString(1, "%" + searchWord + "%");
					stmt1.setInt(2, beginRow);//첫번째 물음표에 들어갈 변수
					stmt1.setInt(3, rowPerPage);//두번째 물음표에 들어갈 변수
				} else if (col.equals("empNo")) {

					stmt1 = conn.prepareStatement(
							"select emp_no, dept_no, from_date, to_date from dept_manager where emp_no like ? order by emp_no asc limit ?,?");
					stmt1.setString(1, "%" + searchNo + "%");
					stmt1.setInt(2, beginRow);//첫번째 물음표에 들어갈 변수
					stmt1.setInt(3, rowPerPage);//두번째 물음표에 들어갈 변수
				}
			}
			...
			while (rs1.next()) {//쿼리 실행 결과 데이터가 있으면 계속 반복
				DeptManager dm = new DeptManager();//쿼리 실행결과 데이터로 리스트 작성
				dm.empNo = rs1.getInt("emp_no");
				dm.deptNo = rs1.getString("dept_no");
				dm.fromDate = rs1.getString("from_date");
				dm.toDate = rs1.getString("to_date");
			...
			ResultSet rs2 = null;
			if (searchWord == "") {
				stmt2 = conn.prepareStatement("select count(*) from dept_manager");//데이터의 갯수를 구하라
			} else {
				if (col.equals("deptNo")) {
					stmt2 = conn.prepareStatement("select count(*) from dept_manager where dept_no like ?");
					stmt2.setString(1, "%" + searchWord + "%");
				} else {

					stmt2 = conn.prepareStatement("select count(*) from dept_manager where emp_no like ?");
		...
		<div class="row">
			...
			<div class="col-sm-8">
				...
				<div>
					<form method="post"
						action="<%=request.getContextPath()%>/deptManager/deptManagerList.jsp">
						<select name="col">
				...
				<table border=1 class="table table-hover">
					<thead class="bg-dark text-light table table-dark ">
						<tr>
							<th>deptNo</th>
							<th>empNo</th>
							<th>from date</th>
				...
				<div class="d-flex justify-content-center">
					<!-- 1페이지로 이동 -->
					<a
						href="<%=request.getContextPath()%>/deptManager/deptManagerList.jsp?currentPage=1&searchWord=<%=searchWord%>&col=<%=col%>"
					...
					%>
					<a
						href="<%=request.getContextPath()%>/deptManager/deptManagerList.jsp?currentPage=<%=currentPage - 1%>&searchWord=<%=searchWord%>&col=<%=col%>"
						class="
			btn btn-outline-primary"><</a>
					...
					%>
					<a
						href="<%=request.getContextPath()%>/deptManager/deptManagerList.jsp?currentPage=<%=i%>&searchWord=<%=searchWord%>&col=<%=col%> "
						class="
			...
			btn btn-outline-primary">></a>
					...
					<!-- 마지막 페이지로 이동 -->
					<a
						href="<%=request.getContextPath()%>/deptManager/deptManagerList.jsp?currentPage=<%=lastPage%>&searchWord=<%=searchWord%>&col=<%=col%>"
						class="
