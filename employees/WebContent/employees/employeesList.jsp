...
<body>
	<div class="container-fluid">
		<%
			...
			System.out.println("임플로이스 검색카테고리.." + col);
			System.out.println("임플로이스 검색단어..." + searchWord);
			...
			ArrayList<Employees> list = new ArrayList<Employees>();//현재 패이지 행을 저장할 변수
			...
			if (searchWord == "") {
				...
				if (col.equals("no")) {
					...
					stmt1 = conn.prepareStatement(
							"select * from employees where emp_no like ? order by emp_no asc limit ?,?");
					...
					stmt1 = conn.prepareStatement(
							"select * from employees where (first_name like ? )or(last_name like ? )order by emp_no asc limit ?,?");
			...
			while (rs1.next()) {//쿼리 실행 결과 데이터가 있으면 계속 반복
				Employees e = new Employees();//쿼리 실행결과 데이터로 리스트 작성
			...
			ResultSet rs2 = null;
			if (searchWord == "") {
				stmt2 = conn.prepareStatement("select count(*) from employees");//데이터의 갯수를 구하라
			} else {
				if (col.equals("no")) {
					

					stmt2 = conn.prepareStatement("select count(*) from employees where emp_no like ?");
					stmt2.setString(1, "%" + searchNo + "%");
				} else {
					stmt2 = conn.prepareStatement(
							"select count(*) from employees where (first_name like ? )or(last_name like ? )");
		...
		<div class="row">
			...
			<div class="col-sm-8">
				...
				<div>
					<form method="post"
						action="<%=request.getContextPath()%>/employees/employeesList.jsp">
						<select name="col">
				...
				<div class="d-flex justify-content-center">
					<!-- 1페이지로 이동 -->
					<a
						href="<%=request.getContextPath()%>/employees/employeesList.jsp?currentPage=1&searchWord=<%=searchWord%>&col=<%=col%>"
					...
					%>
					<a
						href="<%=request.getContextPath()%>/employees/employeesList.jsp?currentPage=<%=currentPage - 1%>&searchWord=<%=searchWord%>&col=<%=col%>"
						class="
			btn btn-outline-primary"><</a>
					...
					%>
					<a
						href="<%=request.getContextPath()%>/employees/employeesList.jsp?currentPage=<%=i%>&searchWord=<%=searchWord%>&col=<%=col%> "
						class="
			...
			btn btn-outline-primary">></a>
					...
					<!-- 마지막 페이지로 이동 -->
					<a
						href="<%=request.getContextPath()%>/employees/employeesList.jsp?currentPage=<%=lastPage%>&searchWord=<%=searchWord%>&col=<%=col%>"
						class="
