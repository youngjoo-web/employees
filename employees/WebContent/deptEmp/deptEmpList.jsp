...
<body>
	<div class="container-fluid">
		<%
			...
			System.out.println("부서별직원 검색키워드.." + col);
			System.out.println("부서별직원 검색단어..." + searchWord);
			...
			int rowPerPage = 20;//한 페이지에 들어갈 데이터 수
			...
			ArrayList<DeptEmp> list = new ArrayList<DeptEmp>();//현재 패이지 행을 저장할 변수
			PreparedStatement stmt1 = null;//쿼리문 저장할 변수
			ResultSet rs1 = null;//쿼리문에 의한 데이터값 저장할 변수
			if (searchWord == "") {
				stmt1 = conn.prepareStatement(
						"select emp_no, dept_no, from_date, to_date from dept_emp order by emp_no asc limit ?,?");
				...
				if (col.equals("deptNo")) {
					stmt1 = conn.prepareStatement(
							"select * from dept_emp where dept_no like ? order by emp_no asc limit ?,?");
					...
					stmt1 = conn.prepareStatement(
							"select * from dept_emp where emp_no like ? order by emp_no asc limit ?,?");
			...
			while (rs1.next()) {//쿼리 실행 결과 데이터가 있으면 계속 반복
				DeptEmp de = new DeptEmp();//쿼리 실행결과 데이터로 리스트 작성
				de.empNo = rs1.getInt("emp_no");
				de.deptNo = rs1.getString("dept_no");
				de.fromDate = rs1.getString("from_date");
				de.toDate = rs1.getString("to_date");
			...
			ResultSet rs2 = null;
			if (searchWord == "") {
				stmt2 = conn.prepareStatement("select count(*) from dept_emp");//데이터의 갯수를 구하라
			} else {
				if (col.equals("deptNo")) {
					stmt2 = conn.prepareStatement("select count(*) from dept_emp where dept_no like ?");
					stmt2.setString(1, "%" + searchWord + "%");
				} else if (col.equals("empNo")) {
					
					stmt2 = conn.prepareStatement("select count(*) from dept_emp where emp_no like ?");
		...
		<div>
			<form method="post"
				action="<%=request.getContextPath()%>/deptEmp/deptEmpList.jsp">
				<select name="col">
		...
		<!-- 1페이지로 이동 -->
		<a
			href="<%=request.getContextPath()%>/deptEmp/deptEmpList.jsp?currentPage=1&searchWord=<%=searchWord%>&col=<%=col%>"
			class="btn btn-outline-primary">첫번째</a>
		...
		%>
		<a
			href="<%=request.getContextPath()%>/deptEmp/deptEmpList.jsp?currentPage=<%=currentPage - 1%>&searchWord=<%=searchWord%>&col=<%=col%>"
			class="
		...
		%>
		<a
			href="<%=request.getContextPath()%>/deptEmp/deptEmpList.jsp?currentPage=<%=i%>&searchWord=<%=searchWord%>&col=<%=col%> " class="
		...
		<!-- 마지막 페이지로 이동 -->
		<a
			href="<%=request.getContextPath()%>/deptEmp/deptEmpList.jsp?currentPage=<%=lastPage%>&searchWord=<%=searchWord%>&col=<%=col%>"
			class="
			btn btn-outline-primary">마지막</a>
			</div>
			<div class="col-sm-2"></div>
			</div>
	</div>
